...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (cl.HasSwitch("server")) {
		...
		base::FileEnumerator scan_enum(scan_dir, true, base::FileEnumerator::FILES,
									   FILE_PATH_LITERAL("*.*"));
		int total_file_count = 0;
		for (base::FilePath name = scan_enum.Next(); !name.empty(); name = scan_enum.Next(), total_file_count++) {
